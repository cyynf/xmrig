    ...
    if (m_thread->algorithm() == xmrig::CRYPTONIGHT) {
        switch (m_state.job.variant()) {
            ...
            case xmrig::VARIANT_1:
                return verify(xmrig::VARIANT_1, test_output_v1);
            case xmrig::VARIANT_XTL:
                return verify(xmrig::VARIANT_XTL, test_output_xtl);
            case xmrig::VARIANT_MSR:
                return verify(xmrig::VARIANT_MSR, test_output_msr);
            default:
                return false;
        }
    }
...
#   ifndef XMRIG_NO_AEON
    if (m_thread->algorithm() == xmrig::CRYPTONIGHT_LITE) {
        switch (m_state.job.variant()) {
            ...
            case xmrig::VARIANT_1:
                return verify(xmrig::VARIANT_1, test_output_v1_lite);
            case xmrig::VARIANT_IPBC:
                return verify(xmrig::VARIANT_IPBC, test_output_ipbc_lite);
            default:
                return false;
        }
    }
    ...
    if (m_thread->algorithm() == xmrig::CRYPTONIGHT_HEAVY) {
        switch (m_state.job.variant()) {
            ...
            case xmrig::VARIANT_XHV:
                return verify(xmrig::VARIANT_XHV, test_output_xhv_heavy);
            default:
                return false;
        }
    }
...
template<size_t N>
void MultiWorker<N>::start(xmrig::Controller *controller) {
    if (m_usec <= 0) {
